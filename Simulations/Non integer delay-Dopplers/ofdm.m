function [Y_nm, snr, s_n,r ]= ofdm(X_nm,sigma,Ns,L,fs,h_gain,h_doppler,strOpt,bTraining,startPhase,samp_const)
...
% ofdm modulator
x_n = sqrt(Ns)*ifft(X_shaped); %ifft along columns
figure(100);
imagesc(abs(x_n));
% add CP
...
% apply channel
r = apply_channel(s_n,fs,h_gain,h_doppler,strOpt,startPhase,samp_const,N);
...
save_noise = [1 1i] * sigma_new*randn(2,numel(r)); % noise
