...
%     h_amp = [1 -a a^2 -a^3] .* exp(1i*2*pi*[0 rand(1,3)]); % Total 4 paths, thhe delyed paths have random phase
    h_amp = [1 1 1 1] .* exp(1i*2*pi*[0 rand(1,3)]); % Total 4 paths, thhe delyed paths have random phase
...
%     F = 180; % (27 m/s / 0.15 meters)
    ...
    h_doppler(h_delays) = [0.55 0.71 0.61 0.41]*F;
...
%     figure; surf(db( H_nm_tr0 - H_nm_tr ) )
    ...
    % plot/debug
%     figure; surf(db( H_nm_tr0 - H_nm_tr_shifted ) )
    
%     figure;
%     subplot(131); surf(db(H_nm_tr0)); subplot(132); surf(db(H_nm_tr)); subplot(133); surf(db(H_nm_tr_shifted));
% 
%     
%     figure;
%     subplot(131); surf(angle(H_nm_tr0)); subplot(132); surf(angle(H_nm_tr)); subplot(133); surf(angle(H_nm_tr_shifted));
%     
    %% ofdm modulator/demodulator
    [Y_nm_ofdm_pd,snr_pd,s_pd,~] = ofdm(X_nm_ofdm_pd,sigma,Ns,L,fs,h_gains,h_doppler,strOpt,1,0.00/fs);
    [Y_nm_ofdm,snr_ofdm,s_t,~] = ofdm(X_nm_ofdm,sigma,Ns,L,fs,h_gains,h_doppler,strOpt,1,0.00/fs);

    %otfs --> ofdm
    [Y_nm,snr_otfs,s_n,r] = ofdm(X_nm,sigma,Ns,L,fs,h_gains,h_doppler,strOpt,1,0.00/fs);

    % reshape
    Y_nm_ofdm_blocks = reshape(Y_nm_ofdm,round(M/Nb),(N*Nb));
    X_nm_ofdm_blocks = reshape(X_nm_ofdm,round(M/Nb),(N*Nb));
    Y_nm_ofdm_blocks_eq = Y_nm_ofdm_blocks;

    % block process ofdm
    [nBlocks, ~] = size(Y_nm_ofdm_blocks);

    for n=1:nBlocks
        % ofdm channel est
        H_nm_ofdm = Y_nm_ofdm_blocks(n,(1:Ns) ) .* conj(X_nm_ofdm_blocks(n,(1:Ns) ));

        H_nm_ofdm = repmat(H_nm_ofdm,1,Nb);
        % ofdm equalize
        Y_nm_ofdm_blocks_eq(n,:) = Y_nm_ofdm_blocks(n,:) .* conj(H_nm_ofdm) ./ (abs(H_nm_ofdm).^2 + sigma.^2);
    end
